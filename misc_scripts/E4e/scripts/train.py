...
def main():
	...
	########## Copied parameters from e4e page
	# Make sure to adjust these parameters to your own if required!
	
	opts.decoder_checkpoint = 'pretrained_models/viton-ckpt-256x256-13.04.2022.pt'

	opts.dataset_type = 'fashion'
	opts.start_from_latent_avg = True
	opts.use_w_pool = True
	opts.w_discriminator_lambda = 0.1
	opts.id_lambda = 0.5
	opts.val_interval = 10000
	opts.max_steps = 200000
	opts.stylegan_size = 256
	opts.stylegan_weights = None,
	opts.workers = 8
	opts.test_batch_size = 8
	opts.test_workers = 4
	opts.exp_dir = 'fashion'
	opts.encoder_type = 'Encoder4Editing'

	if opts.encoder_type == 'Encoder4Editing':
		opts.progressive_start = 20000
	else:
		opts.progressive_start = None
